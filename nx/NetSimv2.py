...
def generate_network(nodes_count):
    # the first step is to randomly generate a graph with nodes and random edges
...
def write_to_file(graph, filename):
    # write the network info to a file
...
def generate_packets(node_count, lambda_param=0.5):
    # generate the packets at sources with Poisson distribution
    packets = []
...
class Router:
    ...
    def service_packets(self):
        ...
        for packet in self.input_queue:
            transmission_time = np.random.exponential(scale=1/self.service_rate)
...
class Simulation:
    ...
    def simulate(self, simulation_time=1000):
        ...
        for router in self.routers.values():
            ...
            self.max_completion_time = max(self.max_completion_time, router.output_queue[-1]['packet']['arrival_time'])
            self.min_completion_time = min(self.min_completion_time, router.output_queue[0]['packet']['arrival_time'])
